...
class User
{
    ...
    {
        $password_reset = "NOT IMPLIMENTED";

        $sql = "INSERT INTO users (`first_name`,`last_name`,`email`,`password`,`password_reset_token`) VALUES ('$first_name','$last_name','$email','$password','$password_reset')";
    ...
    static function addBid(string $task_id, int $amount, string $token): mysqli_result|bool
    {
        ...
        $sql = "INSERT INTO bids (`user_id`,`task_id`,`hash`,`validated`) VALUES ('$user_id','$task_id','$hash','$validated')";
        return Database::query($sql);
    }
    ...
    static function validateBid($bid_id, $user_id, $amount, $existing_hash): mysqli_result|bool
    {
        $hash = hash(hash_type,pepper.$amount);
        ...
        if($hash != $existing_hash){
            ...
            $sql = "UPDATE bids SET validated='1' WHERE user_id='$user_id' AND bid_id='$bid_id'";
            return Database::query($sql);
        }
