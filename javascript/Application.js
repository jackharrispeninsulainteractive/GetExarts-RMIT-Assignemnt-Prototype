class Application {
    ...
    openValidationWindow(id){
        document.getElementById("new-task-window").style.display = "block";
        document.getElementById("bid-validation-id").valueOf().value = id;
    }

    validateBid(){
        let bidValidationAmount = document.getElementById("bid-validation-input").valueOf().value;
        let id = document.getElementById("bid-validation-id").valueOf().value;

        console.log("Validating bid amount: "+bidValidationAmount+" for bid id: "+id)

        this.postFormData(this.domain + this.domainLocation + "/",
            "action=validate-bid" +
            "&bid_id=" + id +
            "&amount=" + bidValidationAmount +
            "&token=" + this.getCookie("token")
        ).then(function (outcome) {
            let response = JSON.parse(outcome);
            if (response.outcome) {
                Application.instance.loadPage("my-bids");
                document.getElementById("new-task-window").style.display = "none";
            } else {
                if (response.error.image !== null) {
                    document.getElementById("new-task-window-error").innerText = response.error.general;
                }
            }
        });
    }
